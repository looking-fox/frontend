const theme = {
  primaryColor: "black",
  primaryText: "white",
  lightGrey: "#f5f5f5",
  red: "#e65f5c",
  green: "#00b88b",
  purple: "#7b16ff",
  flexAllCenter: {
    display: "flex",
    alignItems: "center",
    justifyContent: "center"
  },
  flexEvenly: {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-evenly"
  },
  boxShadow: `-webkit-box-shadow: 0px 0px 3px 2px rgba(145, 145, 145, 0.1);
  -moz-box-shadow: 0px 0px 3px 2px rgba(145, 145, 145, 0.1);
  box-shadow: 0px 0px 3px 2px rgba(145, 145, 145, 0.1);`,
  darkBoxShadow: `-webkit-box-shadow: 0px 0px 2px 2px rgba(145, 145, 145, 0.3);
  -moz-box-shadow: 0px 0px 2px 2px rgba(145, 145, 145, 0.3);
  box-shadow: 0px 0px 2px 2px rgba(145, 145, 145, 0.3);`,
  sideBoxShadow: `-webkit-box-shadow: 10px 10px 5px -9px rgba(145, 145, 145, 0.1);
  -moz-box-shadow: 10px 10px 5px -9px rgba(145, 145, 145, 0.1);
  box-shadow: 10px 10px 5px -9px rgba(145, 145, 145, 0.1);`
};

export default theme;
